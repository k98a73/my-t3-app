...
import type { Todo } from "~/server/type";
...
export function Todo({ todo }: TodoProps) {
  ...
  const { mutate: toggleMutation } = api.todo.toggle.useMutation({
    ...
    onError: (err, is_completed, context) => {
      toast.error(
        `An error occurred when marking todo as ${
  ...
  const { mutate: updateMutation } = api.todo.update.useMutation({
    ...
    onError: (err, updateTodo, context) => {
      toast.error("An error occurred when editing todo");
      console.error(err);
      const previousText = context?.previousTodos?.filter(
  ...
  return (
    <div className="flex items-center justify-between rounded-md border-2 border-gray-one px-5 py-4">
      ...
      <button
        type="button"
        className="group ml-4 flex items-center justify-center rounded-md bg-cream-four p-2 hover:bg-steel-one focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gray-five"
        onClick={() => {
          deleteMutation(id);
        }}
      >
